  ...
  fetchLeaveLine(DateTime formdate, DateTime endDate) async {
    ...
    await leaveService?.getLeaveLine(leave).then((value) {
      ...
      if (value != null && value.length>0) {
        if (value[0].message == null || value[0].message == '') {
  ...
  requestUpdateLeave(String duration, String id) async {
    ...
    if (valid) {
      ...
      await leaveService?.deleteLeave(int.tryParse(id)!).then((data) async{
        if(data){
        await leaveService?.createLeave(leaveRequest,1).then((data) async{
          Get.back();
          if (data.contains("ERROR")) {
            ...
            Get.defaultDialog(title:'Information',content: Text('Successfully Updated!'),confirmTextColor: Colors.white,buttonColor: Color.fromRGBO(63, 51, 128, 1), onConfirm: (){
              Get.back();
              Get.back();
