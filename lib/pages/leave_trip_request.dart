...
class _StateLeaveTripRequest extends State<LeaveTripRequest> {
  ...
  Widget dateWidget(BuildContext context, String date, String date_string) {
    ...
    if (date == 'Start Date') {
      ...
      if (date_string == null) {
        ...
        // controller.calcuinterval(fromData, toDate);
      }
      ...
      builder: (BuildContext? context, Widget? child) {
        return Theme(
          data: ThemeData.light().copyWith(
            dialogBackgroundColor: Color.fromRGBO(60, 47, 126, 1),
            datePickerTheme: const DatePickerThemeData(
                headerForegroundColor: Colors.white,
                shape: OutlineInputBorder(
                    borderRadius: BorderRadius.zero,
                    borderSide: BorderSide.none),
                headerBackgroundColor: Color.fromRGBO(60, 47, 126, 1)),
      ...
      return index == null
          ...
          : Container(
              ...
              child: Image.memory(Base64Decoder().convert(
                  controllerList.leaveList.value[index ?? 0].attachment!)));
    }
  }
  ...
  Widget build(BuildContext context) {
    final labels = AppLocalizations.of(context);
    index = Get.arguments == null ? 0 : Get.arguments;
    if (index == 0) {
    } else {
      starttime =
          controllerList.leaveList.value[index ?? 0].start_date.toString();
      endtime = controllerList.leaveList.value[index ?? 0].end_date.toString();
      starttimeDate = DateTime.parse(
          controllerList.leaveList.value[index ?? 0].start_date.toString());

      endtimeDate = DateTime.parse(
          controllerList.leaveList.value[index ?? 0].end_date.toString());
      controller.durationController.text =
          controllerList.leaveList.value[index ?? 0].duration.toString();
      controller.descriptionController.text =
          controllerList.leaveList.value[index ?? 0].description.toString();
      idData = controllerList
          .leaveList.value[index ?? 0].holiday_status_id!.name
          .toString();
      //
      /*Future.delayed(Duration(seconds: 3), () {
        controller.fetchLeaveLine(starttimeDate, endtimeDate);
        // controller.calcuinterval(starttimeDate, starttimeDate);
        // controller.updateLeaveInterval(index, 3);
      });*/
      controller.is_add_leavelist.value = true;
      controller.leavelLineList.value =
          controllerList.leaveList.value[index ?? 0].leave_line!;

      selectedFromDate = DateTime.parse(
          controllerList.leaveList.value[index ?? 0].start_date.toString());
      selectedToDate = DateTime.parse(
          controllerList.leaveList.value[index ?? 0].end_date.toString());
      if (controllerList.leaveList.value[index ?? 0].attachment!.isNotEmpty)
                ...
                () => Container(
                  ...
                  child: controller.save_btn_show.value
                      ? GFButton(
                          ...
                          text: index == null || index == 0? labels.save : labels.update,
                          blockButton: true,
                          size: GFSize.LARGE,
                        )
                      : new Container(),
